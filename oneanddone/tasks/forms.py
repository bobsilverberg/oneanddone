...
class TaskForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('name', 'short_description', 'execution_time', 'difficulty',
                  ...
                  'is_draft')
        widgets = {
            'name': forms.TextInput(attrs={'size': 100, 'class': 'fill-width', 'required': 'required'}),
            'short_description': forms.TextInput(attrs={'size': 100, 'class': 'fill-width', 'required': 'required'}),
            'instructions': AceWidget(mode='markdown', theme='textmate', width='800px',
                                      height='300px', wordwrap=True,
                                      attrs={'class': 'fill-width', 'required': 'required'}),
            ...
            'team': forms.Select(attrs={'required': 'required'}),
        }
